...
const CreateDetails = () => {
  ...
  const handleSaveDetail = () => {
    ...
    setLoading(true);
    axios
      .post('https://mernprojectbackend-1-9ko5.onrender.com/details', data)
      .then(() => {
        setLoading(false);
        enqueueSnackbar('detail Created successfully', { variant: 'success' });
        navigate('/');
      })
      .catch((error) => {
        setLoading(false);
  ...
  return (
    <div className='p-4 h-screen bg-gradient-to-br from-blue-500 to-purple-600'>
      <BackButton />
      <h1 className='text-3xl my-4 text-white'>Create Detail</h1>
      {loading ? <Spinner /> : ''}
      <div className='flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto bg-white bg-opacity-70'>
        ...
        <button className='p-2 bg-sky-300 m-8' onClick={handleSaveDetail}>
          Save
        </button>
